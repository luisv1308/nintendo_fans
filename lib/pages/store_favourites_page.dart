...
class StoreFvaouritesPage extends StatefulWidget {
  final scaffoldKey;
  final storage = new FlutterSecureStorage();
  final StoreService service = StoreService(RestClient());

  StoreFvaouritesPage(this.scaffoldKey);

  @override
  _StoreFvaouritesPageState createState() => _StoreFvaouritesPageState();
}

class _StoreFvaouritesPageState extends State<StoreFvaouritesPage>
    with AutomaticKeepAliveClientMixin<StoreFvaouritesPage> {
  var list = List();
  var userID;
  ...
  @override
  void initState() {
    games = productBloc.getFavouritesList().then((res) {
      return res;
    });
    super.initState();
  }
  ...
  Future<List<Game>> load() {
    if (productBloc.meta == null) {
      ...
      return games = productBloc
          .moreProducts(
              'http://phplaravel-175876-509694.cloudwaysapps.com/api/favourites?page=1')
          .then((res) {
        return res;
      });
    } else {

...
class StoreComingSoonPage extends StatefulWidget {
  final scaffoldKey;
  final storage = new FlutterSecureStorage();
  final StoreService service = StoreService(RestClient());

  StoreComingSoonPage(this.scaffoldKey);

  @override
  _StoreComingSoonPageState createState() => _StoreComingSoonPageState();
}

class _StoreComingSoonPageState extends State<StoreComingSoonPage>
    with AutomaticKeepAliveClientMixin<StoreComingSoonPage> {
  var list = List();
  var userID;
  ...
  @override
  void initState() {
    games = productBloc.getComingSoonList().then((res) {
      return res;
    });
    super.initState();
  }
  ...
  Future<List<Game>> load() {
    if (productBloc.meta == null) {
      ...
      return games = productBloc
          .moreProducts(
              'http://phplaravel-175876-509694.cloudwaysapps.com/api/comingSoonGames?page=1')
          .then((res) {
        return res;
      });
    } else {
